...
public class TypeCasting {
    ...
    public static void main(String[] args) {
        ...
        int i2 = 100;
        byte b2 = (byte) i2; // int is bigger than byte, so we must cast. casting is converting a datatype. We cast the int type - i2 to a byte type using (byte)

        int i3 = 500;
        long l1 = i3; // int is smaller than long, so it will automatically cast.

        long l2 = 200;
        int i4 = (int)l2; //im trying to go from long to int. long is bigger than int, so I much cast it
